...
const resolvers = {
  Query: {
     ...
     */
    movies: async (_, { limit, offset, genre }) => {
      let results = await Movie.find()

      if (genre) {
        results = results.filter((movie) => {
          try {
            const parsedGenres = JSON.parse(movie.genre.replace(/'/g, '"'))
            return parsedGenres.some((g) => g.name === genre)
          } catch (e) {
            console.error('Genre parse error:', e)
            return false
          }
        })
      }
